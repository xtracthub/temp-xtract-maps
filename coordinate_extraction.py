...
def max_by_val(d, dir):
    ...
    dir (str): The direction you want to find the largest coordinate from
    ('N', 'E', 'S', 'W' )
...
def min_by_val(d, dir):
    ...
    d (dictionary): A direction-to-location-to-coordinate dictionary as
    returned by get_coordinates_from_text.
    dir (str): The direction you want to find the smallest coordinate from
    ('N', 'E', 'S', 'W' )
...
def coord_to_abs(value, direction, flip_lon=False, to_int=False):
    """Converts a number and direction into a positive or negative integer
    (e.g. from (50, "N") and (100, "W") to 50 and -100 resp).

    Parameters:
    value (int): Latitude or longitude of direction.
    direction (str): Direction of value ('N', 'E', 'S', 'W')
    flip_lon (bool): If true, 'E' is treated as negative and 'W' is treated
    positive, otherwise 'E' is treated as positive and 'W' is treated as
    negative.
...
def coord_from_abs(value, orientation, flip_lon=False, to_int=False):
    """Converts a number and an orientation and converts it into a positive or
    negative integer (e.g. from (50, "N") and (100, "W") to 50 and -100 resp).
...
def any_coord_equal(coords1, coords2, orientation):
    """Checks whether the position or the corresponding x or y coordinate
    of 2 coordinate pairs is the same or not. Note: If both coordinates are
    empty, returns True.
    ...
    y-coordinates). Accepts 'Latitude', 'Vertical', 'Longitude', 'Horizontal'.

    Returns:
    (bool): Whether coordinate pairs or corresponding coordinates are the same.
    """
    if coords1 == coords2:
        return True
    pos1, v1, d1 = coords1
...
def valid_coordinate(coord):
    ...
    Parameter:
    coord (tuple): 2-tuple of latitude or longitude value and 'N', 'E', 'S',
    'W'.
...
def valid_coordinate_pair(pair):
    ...
    Longitude and latitude should be a 2-tuple of latitude or longitude value
    and 'N', 'E', 'S', 'W'.

    Return:
    (list(bool)): List of whether longitude direction, latitude direction,
    longitude coordinate, and latitude coordinate are valid.
    """
    lon, lat = pair
    return all([
        lon[1].upper() in 'EW',
